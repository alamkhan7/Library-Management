	...
	if ( $std_query_result = mysqli_query($conn,$std_query) )
	{
		if ($std_row = mysqli_fetch_assoc($std_query_result))
		{
			...
			if($std_row['Student_Reg'] > 0)
			{
				...
				mysqli_free_result($std_query_result);
				return true ;
			}
			else
			{
...
if (isset($_POST['add-book-id']) && isset($_POST['reg_no']) && isset($_POST['submit']))
{
	 ...
	 if (!empty($book_id) && !empty($reg_no))
	 {
	 	if ($no_copies=get_book_data($book_id,$reg_no))
	 	{
			...
			{
				$book_message = "Book Successfuly Issue!";
				mysqli_free_result($issue_query_result);
			}
			else		
				$book_message = "Error: Issue Book!";
		}
	 	else
	 		$book_message ="Sorry Book Copies Not Available";
	 }
	 	
	 else
		$book_message = "Fill Please The Form!" ;	
}
...
if (isset($_POST['add-book-id']) && isset($_POST['reg_no']) && isset($_POST['submit']))
{
	if (!empty($book_id) && !empty($reg_no))
	{
		header('Location: ../student-search.php?addmessage='.$book_message.'&reg='.$reg_no) ;
	}
}
elseif (isset($_POST['delete-book-id']) && isset($_POST['reg_no']) && isset($_POST['submit']))
{
	if (!empty($book_id) && !empty($reg_no))
	{
		header('Location: ../student-search.php?deletemessage='.$delete_message.'&reg='.$reg_no) ;
	}
}
else
	header('Location: ../student-search.php?reg='.$reg_no) ;	
